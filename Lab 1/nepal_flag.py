...
def draw_triangles_round(n, height, startpoint, base_length, angle):
    """
    Draw the Triangles around Sun and Moon to make them look like Sun and Moon.

    Parameters:
    n (int): The number of triangles to draw.
    height (int): The height of each triangle.
    startpoint (tuple): The starting point of the first triangle.
    base_length (int): The base length of each triangle.
    angle (float): The angle by which each triangle is rotated.
    ...
    for i in range(n):
        ...
        outer_point = (startpoint[0] + height, startpoint[1] - base_length/2)
        rotated_end_point = rotation(endpoint, startpoint, -(i * angle))
        rotated_outer_point = rotation(outer_point, startpoint, -(i * angle))
        initial_poly = pygame.draw.polygon(screen, WHITE, (startpoint, rotated_end_point, rotated_outer_point))
